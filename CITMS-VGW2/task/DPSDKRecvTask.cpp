...
// 日期, 作者, 变更内容
// **********************************************************************
#include "DPSDKRecvTask.h"
#include "MiscTool.h"
#include "TimeConsuming.h"

using namespace ScheduleServer;

int32_t CDPSDKRecvTask::_sdk_handler = -1;
bool CDPSDKRecvTask::_sdk_initialized = false;
CSSMutex CDPSDKRecvTask::_sdk_init_mutex;

CDPSDKRecvTask::CDPSDKRecvTask(SDK_RECV_TASK_INFO& task_info):
...
int CDPSDKRecvTask::connect_device()
{
	...
	int ret = -1;
	{
		...
		ret = DPSDK_GetRealStream( _sdk_handler, _seq, &stream_info, CDPSDKRecvTask::MediaDataCallback, (void*)this);
	}
	
	return ret;
}

void CDPSDKRecvTask::on_recv_packet(unsigned char* data, int len)
{
	...
	else if(NAL_SLICE == type || NAL_SPS == type)
	{
		...
		if(_frame_pos) on_recv_frame(_frame, _frame_pos);
...
int32_t DPSDK_CALLTYPE CDPSDKRecvTask::MediaDataCallback(int32_t nPDLLHandle, int32_t nSeq, int32_t nMediaType, const char* szNodeId, int32_t nParamVal, char* szData, int32_t nDataLen, void* pUserParam)
{
	...
	if(40 >= nDataLen)
	{
		TRACE("\n<SHORT %d %x %x %x %x %x", nMediaType, szData[0], szData[1], szData[2], szData[3], szData[4]);
		return 0;
	}

	CDPSDKRecvTask* task = (CDPSDKRecvTask*)pUserParam;
...
SS_Error CDPSDKRecvTask::on_done()
{
	_task_info.clear();
