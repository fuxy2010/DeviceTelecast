...
int osip_from_to_str(const osip_from_t * from, char **dest)
{
	...
	*dest = buf;
	...
	//fym <sip:111111@192.168.1.6:10000>;transport=tcp => <sip:111111@192.168.1.6:10000;transport=tcp>
	if(NULL != strstr(buf, ";transport") && '>' != *(buf + strlen(buf) - 1))
	{
		int length = strlen(buf);
		int pos = strstr(buf, ";transport") - buf;

		printf("\ncontact from: %s", buf);

		memcpy(buf + pos - 1, buf + pos, length - pos);		
		*(buf + length -1) = '>';
