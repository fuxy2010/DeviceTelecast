...
{
    int         nAreaID;                // 输出区域流水号
    char     strAreaName[128];            // 输出区域名称
    int       nOutputAreaIndex;        // 输出区域索引
    BOOL     bUsedForAlarm;            // 是否用于报警
    int         nDecIndex;                // 关联解码通道
...
{
    int      nVideoOutID;        // 视频输出通道流水号
    int         nDecodeResID;        // 解码资源流水号
    int       nOutputChanIndex;    // 输出通道索引
    int         nType;                // 输出通道类型（0-BNC；1-VGA）
    char     strVideoOutName[128];    // 输出通道名称
    char     strVideoOutPos[128];    // 输出窗口位置
    BOOL     bOutput;            //    输出标记（0-不输出；1-输出）
    int         nDivType;            // 画面分割类型（0-1画面；1-4画面；2-9画面；3-16画面）
...
{
    // IVMS5000 逻辑 已经废弃
    const char * const screenID = "screenID";                   //监视屏ID
    const char * const MonitorName = "MoniterName";             //监视屏名称

    // IVMS8600 版本
    const char * const TvwallID = "TvwallID";                   // 电视墙ID
    const char * const DlpID = "DlpID";                         // 大屏ID
    const char * const WindowID = "WindowID";                   // 监视屏ID
    const char * const AreaID = "AreaID";                       // 区域ID
}
...
//{
//	int		iAlarmType;		//报警类型
//	char	szAlarmCode[TMCP_MAX_LEN_DESCRIPTION_128];		//报警编号
//	char	szObjName[TMCP_MAX_LEN_DESCRIPTION_128];		//报警对象
//	char	szAlarmEvent[TMCP_MAX_LEN_DESCRIPTION_128];		//报警事件
//	char	szStartTime[TMCP_MAX_LEN_DESCRIPTION_128];		//报警开始时间
//	char    szEndTime[TMCP_MAX_LEN_DESCRIPTION_128];		//报警结束时间
//
