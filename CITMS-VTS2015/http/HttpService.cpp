...
private:
	...
	string on_livecast_start(string uri)
	{
		...
		//http:/127.0.0.1:80/api/startrealplay?connectstring=rtsp%3A%2F%2Fadmin%3Aadmin%40192.168.7.113%2Fcam%2Frealmonitor%3Fchannel%3D1%26subtype%3D0&_=149846188816

		string url = "";
#if 1
		size_t pos = uri.find("connectstring=") + 14;
		if(string::npos == uri.find("&_="))
			url = uri.substr(pos, uri.length() - pos);
		else
		{
			url = uri.substr(pos, uri.find("&_=") - pos);
		}
#else//rtsp url中必须全部使用转义字符
		Poco::URI orig_uri(uri);
		auto paramsVector = orig_uri.getQueryParameters();
		for (auto i = 0; i < paramsVector.size(); ++i)
		{
			std::string key = Poco::toLower(paramsVector[i].first);
			std::string val = paramsVector[i].second;

			if(string::npos != Poco::toLower(val).find("rtsp"))
				url = val;
		}
#endif
		...
		if(false == res.num.empty())
		{
			...
			json_value["StreamUrl2"] = Json::Value(res.url2);
		...
		else
		{
			...
			//RTSP
			{
				...
				//task_info.rtsp_url = "rtsp://admin:admin@192.168.7.113/cam/realmonitor?channel=1&subtype=0";
			...
			res.url = hls_url;
			res.url2 = rtmp_url;
			res.expire = MiscTools::parse_type_to_string<int>(DEFAULT_EXPIRE);
			...
			json_value["ExpireTime"] = Json::Value(MiscTools::parse_type_to_string<int>(DEFAULT_EXPIRE));
	...
	{
		CSSLocker lock(&_livecast_mutex);
		//http://127.0.0.1:80/api/startrealplay?connectstring=dsssdk://fym:88888888@192.168.0.190:9000/params?cameraid=1000004&channel=0&fps=25
		//dsssdk://fym/88888888@192.168.0.190:9000/params?cameraid=1000004&channel=0&fps=25

		//http://127.0.0.1:80/api/startrealplay?connectstring=ivms8600://fym:88888888@192.168.0.190:9000/params?dev=hk&cameraid=1000004&fps=25
		//ivms8600://fym/88888888@192.168.0.190:9000/dev=hk&params?cameraid=1000004&fps=25

		//http://127.0.0.1:80/api/startrealplay?connectstring=hiksdk://admin:citmsb7@192.168.0.237:8000/params?channel=2
		//hiksdk://admin/citmsb7@192.168.0.237:8000/params?channel=2

		//http://127.0.0.1:80/api/startrealplay?connectstring=ivms8600://admin:Hsjj1590@192.168.5.211:8011/params?dev=hk&cameraid=358&fps=25&_=1498739820051
		//ivms8600://admin:Hsjj1590@192.168.5.211:8011/params?dev=hk&cameraid=358&fps=25&_=1498739820051
		...
		else
		{
			...
			//SDK
			{
				...
				{
					rtmp_push_task->shutdown();
					return return_response(2001, "Failed in creating sdk session, ret code: " + MiscTools::parse_type_to_string<int>(sdk_recv_task->get_connect_ret()), "");
				}
	...
	{
		CSSLocker lock(&_vod_mutex);		
		//http://127.0.0.1:80/api/startvodplay?connectstring=dsssdk://fym/88888888@192.168.0.190:9000/params?cameraid=1000004&channel=0&begintime=1486569600&endtime=1486655999&fps=25
		//dsssdk://fym/88888888@192.168.0.190:9000/params?cameraid=1000004&channel=0&begintime=1486569600&endtime=1486655999&fps=25

		//http://127.0.0.1:80/api/startvodplay?connectstring=ivms8600://fym/88888888@192.168.0.190:9000/params?dev=hk&cameraid=1000004&condition=1&begintime=1482474300&endtime=1482474397&fps=25
		//ivms8600://fym/88888888@192.168.0.190:9000/params?dev=hk&cameraid=1000004&condition=1&begintime=1482474300&endtime=1482474397&fps=25
		...
		else
		{
			...
			//SDK
			{
				...
				if (NULL != strstr(url.c_str(), "dsssdk") || NULL != strstr(url.c_str(), "DSSSDK"))
					...
					sdk_recv_task = new CHIKSDKVodPullTask(task_info);
...
protected:
	virtual void run()
	{
		...
		if(string::npos != uri.find("startrealplay"))
		{
			...
			else if(string::npos != uri.find("ivms8600://"))
				res = on_livecast_start_sdk(uri);
			else if (string::npos != uri.find("hiksdk://"))
				res = on_livecast_start_sdk(uri);
		}
