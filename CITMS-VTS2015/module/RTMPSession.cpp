...
int CRTMPSession::connect(char* url)
{
	...
	if(true)
	{
		...
		pack.m_nBodySize = 4;
		int nVal = 512000;// 1024;// 512000;//2560000;//1280000;// 655360;//512000;//128000;//65536;//希望指定的大小
...
int CRTMPSession::input_video_packet(unsigned char* data, int length, unsigned long time_offset)
{
	...
	//TRACE("\n------------------ <Frame %d time: %d %d ------ ", type, time_offset, length);
	...
	else if(NAL_PPS == type)
	{
		...
		ret = send_video_sps_pps(_sps, _sps_size, _pps, _pps_size, time_offset);
	}
	else
	{
...
int CRTMPSession::send_video_sps_pps(unsigned char* sps, int sps_size, unsigned char* pps, int pps_size, unsigned long time_offset)
{
	...
	video_packet->m_nTimeStamp = time_offset;
...
int CRTMPSession::send_video_packet(unsigned char* data, unsigned long length, NAL_TYPE type, unsigned long time_offset)
{
	...
	/*调用发送接口*/
	{
		...
		if(RTMP_IsConnected(_rtmp))
		{
			...
			{
				TRACE("\nSE %d", ret);
				//LOG_WRITE("RTMP PUSH fail: ", 1, false);
			}
			//TRACE("\nS %d", ret);
