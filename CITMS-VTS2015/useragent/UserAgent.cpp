...
SS_Error CUserAgent::add_sample_video_packet(VIDEO_PACKET_PTR packet)
{
...

	{
		...
		if(false)
		{
			FILE* f = fopen("d:\\hk_pure2.dat", "ab+");
...
SS_Error CUserAgent::add_dev_video_frame(VIDEO_FRAME_PTR frame)
{
	//插入队列前必须先判断否则排序报错!!!
	if (NULL == frame.frame)
		return SS_InsertMediaDataFail;
	...
	{
		CSSLocker lock(&_dev_video_frame_list_mutex);

		if(false)//if (_max_video_packet_num < _dev_video_frame_list.size())
		{
			CMemPool::free_video_frame(_dev_video_frame_list.front());
			_dev_video_frame_list.pop_front();

			TRACE("~");
		}

		//TRACE("\n<ADD FRAME %ld, %d> ---------", frame.frame->timestamp, _dev_video_frame_list.size());

		_dev_video_frame_list.push_back(frame);
	}
...
VIDEO_PACKET_PTR CUserAgent::fetch_sample_video_packet()
{
	...
	if(100 < packet_len) _next_fetch_video_frame_timestamp += _info.video_frame_interval;// FILE_VIDEO_FRAME_INTERVAL;
	return packet_ptr;
}
#else
//取最早的视频数据包
...
{
	{
		...
		//if (100 < packet.packet->payload_size) _next_fetch_video_frame_timestamp += _info.video_frame_interval;// FILE_VIDEO_FRAME_INTERVAL;
...
VIDEO_FRAME_PTR CUserAgent::fetch_dev_video_frame()
{
	{
		CSSLocker lock(&_dev_video_frame_list_mutex);
		...
		//if (true == _dev_video_frame_list.empty()) return frame_ptr;
		if (3 >= _dev_video_frame_list.size()) return frame_ptr;

		if (!_next_fetch_video_frame_timestamp) _next_fetch_video_frame_timestamp = timeGetTime();

		if (_next_fetch_video_frame_timestamp > timeGetTime()) return frame_ptr;

		VIDEO_FRAME_PTR frame = _dev_video_frame_list.front();//*(_video_packet_list.begin());
		_dev_video_frame_list.pop_front();

		//if (100 < packet.packet->payload_size) _next_fetch_video_frame_timestamp += _info.video_frame_interval;// FILE_VIDEO_FRAME_INTERVAL;
		_next_fetch_video_frame_timestamp += _info.video_frame_interval;
...
SS_Error CUserAgent::add_dev_video_packet(unsigned char* packet, unsigned long length)
{
	if (!length) return SS_NoErr;
	...
	{
		CSSLocker lock(&_dev_video_packet_list_mutex);

		//if(_max_video_packet_num < _sample_video_packet_list.size())
		if(false)
		{
			CMemPool::free_video_packet(_dev_video_packet_list.front());
			_dev_video_packet_list.pop_front();

			TRACE("~");
		}

		_dev_video_packet_list.push_back(packet_ptr);
	}

	return SS_NoErr;
}

VIDEO_PACKET_PTR CUserAgent::fetch_dev_video_packet()
{
	CSSLocker lock(&_dev_video_packet_list_mutex);

	VIDEO_PACKET_PTR packet_ptr;

	if (true == _dev_video_packet_list.empty()) return packet_ptr;
	//if (3 >= _dev_video_packet_list.size()) return packet_ptr;

	VIDEO_PACKET_PTR packet = _dev_video_packet_list.front();
	_dev_video_packet_list.pop_front();
