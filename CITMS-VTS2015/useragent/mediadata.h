...
//#define MAX_VIDEO_PACKET_SIZE_BYTE	(65536 * 1)
//#define MAX_VIDEO_FRAME_SIZE_BYTE		(65536 * 20)
#define MAX_VIDEO_PACKET_SIZE_BYTE		(10240)
//#define MAX_VIDEO_FRAME_SIZE_BYTE		(65536 * 10)
...
namespace ScheduleServer
{
	...
	typedef struct tagVIDEO_PACKET
	{
		...
		//视频RTP数据包（含RTP包头）
		BYTE* payload;//BYTE payload[MAX_VIDEO_PACKET_SIZE_BYTE];
	...
	typedef struct tagVIDEO_FRAME
	{
		...
		DWORD			sequence;//RTP包序列号
		unsigned long	timestamp;//RTP包时戳
		unsigned short	type;//帧类型0-未知, 1-I帧，2-非I帧

		BYTE* payload;//BYTE payload[MAX_VIDEO_FRAME_SIZE_BYTE];

		tagVIDEO_FRAME() : ua_id(0), payload_size(0), sequence(0), timestamp(0), type(0)
		{
		}
		...
		bool operator < (tagVIDEO_FRAME const& _packet) const
		{
			if (timestamp < _packet.timestamp)
				return true;
			else if (timestamp == _packet.timestamp)
	...
	//视频RTP数据包
	typedef struct tagVIDEO_FRAME_PTR
	{
		VIDEO_FRAME* frame;

		tagVIDEO_FRAME_PTR() : frame(NULL)
		{
		}
		...
		bool operator < (tagVIDEO_FRAME_PTR const& _frame_ptr) const
		{
			if (NULL == frame)
				return true;
	...
	//typedef struct tagSINGLETON_VIDEO_FRAME_POOL {} SINGLETON_VIDEO_FRAME_POOL;
	//typedef boost::singleton_pool<SINGLETON_VIDEO_FRAME_POOL, sizeof(VIDEO_FRAME)> video_frame_pool;
	...
	public:
		...
		//////////////////////////////////////////////////////////////////////////
		static void malloc_video_packet(VIDEO_PACKET_PTR& packet_ptr, unsigned long size)
		{
			/*if(NULL != packet_ptr.packet)
			...
			packet_ptr.packet->payload = new BYTE[size];
			::memset(packet_ptr.packet->payload, 0, size);
		}
		...
		static void free_video_packet(VIDEO_PACKET_PTR& packet_ptr)
		{
			...
			delete packet_ptr.packet->payload;
			packet_ptr.packet->payload = NULL;
		...
		//////////////////////////////////////////////////////////////////////////
		static void malloc_video_frame(VIDEO_FRAME_PTR& frame_ptr, unsigned long size)
		{
			if (NULL != frame_ptr.frame)
				delete frame_ptr.frame;

			frame_ptr.frame = new VIDEO_FRAME;
			::memset(frame_ptr.frame, 0, sizeof(VIDEO_FRAME));

			frame_ptr.frame->payload = new BYTE[size];
			::memset(frame_ptr.frame->payload, 0, size);
		}

		static void free_video_frame(VIDEO_FRAME_PTR& frame_ptr)
		{
			if (NULL == frame_ptr.frame)
				return;

			delete frame_ptr.frame->payload;
			frame_ptr.frame->payload = NULL;
