...
namespace ScheduleServer
{
	...
	public:
		...
		//UA登记
		SS_Error reg_ua(const unsigned long& id, USER_AGENT_TYPE type, unsigned long video_frame_interval = 0)
		{
			if(!id)
				return SS_NoErr;
			...
			if(_ua_map.end() == _ua_map.find(id))
			{
				...
				info.id = id;
				info.type = type;
				info.video_frame_interval = video_frame_interval;
			...
			else
			{
				...
				ua->_info.type = type;
				if(video_frame_interval) ua->_info.video_frame_interval = video_frame_interval;
			}
	...
	public:
		void add_log(string log)
		{
			...
			CEdit* edit = (CEdit*)(AfxGetApp()->GetMainWnd()->GetDlgItem(IDC_EDIT_HTTP_SERVICE));

			if (100 < edit->GetLineCount()) _status = "";

			_status = log + "\r\n\r\n" + _status;

			edit->SetWindowText(_status.c_str());
		}
